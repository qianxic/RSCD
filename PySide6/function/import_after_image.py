...
class ImportAfterImage:
    ...
    def import_after_image(self):
        ...
        if file_path:
            self.navigation_functions.log_message(f"已选择后时相影像: {file_path}")
            
            # 保存图像到指定目录
            saved_path = self.save_image_to_dir(file_path, "后时相")
            if saved_path:
                self.navigation_functions.file_path_after = saved_path
            else:
                # 如果保存失败，仍然使用原始路径
